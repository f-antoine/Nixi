...
namespace NixiTestTools.TestInjectorElements.Relations.RootRelations
{
    ...
    internal sealed class RootRelationHandler
    {
        ...
        #region Add
        /// <summary>
        /// Create a parent root relation without children (component is stored at parent level)
        /// </summary>
        ...
        internal void AddParentWithoutChildren(string parentName, Component parentComponent)
        {
        ...
        /// <summary>
        /// Create a parent root relation with its first child (component is stored at child level)
        /// </summary>
        /// <param name="parentName">Parent name of the root relation</param>
        ...
        internal void AddParentWithChildren(string parentName, string childName, Component childComponent)
        {
        ...
        internal void AddComponentIntoParentRelation(RootRelation parentRelation, Component componentToAdd)
        {
            ...
            {
                throw new NotImplementedException($"Cannot add component of type {componentToAddType}, with name {componentToAdd.name} on " +
                                                  $"ParentRootRelation with parentName {parentRelation.Parent.Name}, because a component " +
                                                  $"with this type already exists");
            }

            parentRelation.Parent.AddComponent(componentToAdd);
            RefreshChildrenTransformOfParentRelation(parentRelation);
        }
        ...
        private void RefreshChildrenTransformOfParentRelation(RootRelation parentRelation)
        {
            foreach (ComponentsWithName child in parentRelation.Children)
            {
        ...
        /// <param name="componentToAdd">Component to add to the list of children</param>
        internal void AddAndLinkChildToParent(RootRelation parentRelation, string childName, Component componentToAdd)
        {
            ...
            // Set to parent because RootObject is parent of GameObjectNameToFindName
        ...
        /// <summary>
        /// From child, add component if not already presents in its list of component and link it to the same parent component if it exists
        /// </summary>
        /// <param name="parentRelation">Parent root relation on which the operation is applied</param>
        ...
        internal void UpdateAndLinkChildToParent(RootRelation parentRelation, ComponentsWithName child, Component componentToAdd)
        {
            ...
            {
                throw new NotImplementedException($"Cannot add component of type {componentToAddType}, with name {componentToAdd.name} on " +
                                                  $"ChildRootRelation with parentName {parentRelation.Parent.Name} and childName {child.Name}, " +
                                                  $"because a component with this type already exists");
            }

            // Set to parent because RootObject is parent of GameObjectToFindName
            Component parentComponent = parentRelation.Parent.Components.FirstOrDefault();
            if (parentComponent != null)
            {
        ...
        {
            return parentRelation.Children.SingleOrDefault(x => x.Name == childName);
        }
        ...
        internal IEnumerable<Component> GetComponentsFromRelation(string parentName, string childName)
        {
            ...
            ComponentsWithName child = parentRelation.Children.SingleOrDefault(x => x.Name == childName);
            if (child == null)
                return null;
